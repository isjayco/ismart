...
function send_mail($name_receiver, $email_receiver, $subject, $content) {
    ...
    try {
        ...
        // echo "Gửi thành công";
        // $id_allow = md5(rand(1,1000).time());
        // $_SESSION['id_allow'] = $id_allow;// khởi động session để kiểm tra hợp lệ
        echo "<div class='wp-process'><div class='box-info'><div class='checkout-success'><span class='fa fa-check'></span><h4>Đặt hành thành công</h4></div><a href='?'>Tiếp tục mua sắp</a></div></div>"; 
    } catch (Exception $e) {
